...
- (BOOL)shouldHandleURL:(NSURL *)URL {
  ...
  NSURL *standardizedDestinationURL = [_request.destinationURL standardizedURL];

  BOOL isRedirectUrl = OIDIsEqualIncludingNil(standardizedURL.scheme, standardizedRedirectURL.scheme) &&
  ...
  BOOL isDestinationUrl = OIDIsEqualIncludingNil(standardizedURL.scheme, standardizedDestinationURL.scheme) &&
      OIDIsEqualIncludingNil(standardizedURL.user, standardizedDestinationURL.user) &&
      OIDIsEqualIncludingNil(standardizedURL.password, standardizedDestinationURL.password) &&
      OIDIsEqualIncludingNil(standardizedURL.host, standardizedDestinationURL.host) &&
      OIDIsEqualIncludingNil(standardizedURL.port, standardizedDestinationURL.port) &&
      OIDIsEqualIncludingNil(standardizedURL.path, standardizedDestinationURL.path);

  return (isRedirectUrl || isDestinationUrl);
}
